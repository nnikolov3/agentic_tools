...
class CodeQualityEnforcer:
    ...
    async def run(self, directory_path: str) -> None:
        ...
        """
        logger.info(f"Starting code quality enforcement for directory: {directory_path}")
        ...
        if not source_files:
            logger.warning(f"No source files found in {directory_path} matching extensions: {self.file_extensions}")
            return
    ...
    async def _process_file(self, file_path: Path) -> None:
        ...
        if self.run_pre_validation:
            ...
            if not pre_validation.is_valid:
                logger.warning(f"File is 'dirty' before LLM transformation. Skipping {file_path_str}.")
        ...
        commentator_agent = self._create_agent(self.commentator_agent_name, current_content)
        transformed_content: str = await commentator_agent.run_agent()
        
        if not transformed_content:
            logger.error(f"Commentator returned empty content for {file_path_str}. Skipping save.")
            return
        ...
        for attempt in range(1, self.max_fix_attempts + 1):
            ...
            if attempt >= self.max_fix_attempts:
                logger.error(f"Validation failed after {self.max_fix_attempts} attempts. Discarding changes for {file_path_str}.")
                logger.debug(f"Final errors:\n{validation_result.errors}")
                return

            logger.warning(f"Validation failed (Attempt {attempt}/{self.max_fix_attempts}). Invoking 'developer' agent to fix...")
            
            # Construct prompt for developer agent
            fixer_prompt = self._create_fixer_prompt(current_content, validation_result.errors)
            
            # Invoke 'developer' Agent
            developer_agent = self._create_agent(self.developer_agent_name, fixer_prompt)
            fixed_content: str = await developer_agent.run_agent()
            
            if not fixed_content:
                logger.error(f"Developer agent returned empty content on attempt {attempt}. Terminating fix loop.")
                return
            
            current_content = fixed_content
            
        # Should not be reached, but as a safeguard
        logger.error(f"Fixer loop exited unexpectedly for {file_path_str}. Discarding changes.")
