...
@patch("src.readme_writer_tool._find_source_files")
@patch("src.readme_writer_tool.read_file_head")
@patch("pathlib.Path.iterdir")
@patch("pathlib.Path.exists")
def test_execute_success(
    mock_path_exists: MagicMock,
    mock_path_iterdir: MagicMock,
    mock_read_file_head: MagicMock,
    mock_find_sources: MagicMock,
    ...
    # Mock the file path object and its stat method
    mock_stat = MagicMock()
    mock_stat.st_size = 100
    mock_file_path = MagicMock(spec=Path)
    mock_file_path.stat.return_value = mock_stat
    mock_file_path.relative_to.return_value = Path("src/fake.py")

    # Mock the functions called by _assemble_sources
    mock_find_sources.return_value = [mock_file_path]
    mock_read_file_head.return_value = "fake file content"
...
def test_readme_writer_tool_follows_design_principles(
    mock_configurator: MagicMock,
) -> None:
    ...
    docstring = readme_writer.__doc__
    assert docstring is not None
    assert "intelligent readme writer" in docstring.lower()
    assert "generates excellent documentation" in docstring.lower()

    # Check that methods have descriptive names and follow explicit over implicit principle
    method_names = [
        method
        for method in dir(readme_writer)
        if callable(getattr(readme_writer, method)) and not method.startswith("__")
    ]
    expected_methods = [
        "execute",
        "_load_inputs",
